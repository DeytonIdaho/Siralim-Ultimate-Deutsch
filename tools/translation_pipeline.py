...
def fix(en,de):
 ...
 # Reviewed gameplay consistency: debuff potency => Effektstärke.
 if re.search(r"(?:debuff|CONDNAME_DEBUFF).{0,100}(?:potency|potent)|(?:potency|potent).{0,100}(?:debuff|CONDNAME_DEBUFF)",en,re.I):
  out=re.sub(r"Wirksamkeit","Effektstärke",out)
 # Equipment spell potency is spell power even when DEF replaces INT as scaling stat.
 if "The potency is based on the caster's {STAT_defense} instead of {STAT_intelligence}" in en:
  out=re.sub(r"Wirksamkeit","Zaubermacht",out)
  out=out.replace("Der Klasse des Zauberwirkers wird zu","Die Klasse des Zauberwirkers wird zu")
  out=out.replace(", es erhält {CONDNAME_",", der Zauberwirker erhält {CONDNAME_")
  out=out.replace(", erhält {CONDNAME_",", der Zauberwirker erhält {CONDNAME_")
 # Specific consistency leftovers.
 ...
 # Earlier reviewed fixes retained.
...
def exception(en,term=None):
 ...
 return False
def false_token(en):return (en.startswith("\\n\\nYou need 5 of these materials")or en.startswith("\\n\\nYou need 3 of these materials")or en=="Balance In All Things"or en.startswith("A random enemy recovers a large amount of {STAT_health}")or en.startswith("AT THE END OF YOUR CREATURES' TURNS, THEY'LL SAY"))
def main():
