...
def fix(en,de):
    ...
    out=de
    # Reviewed terminology and obvious orthography in UI strings.
    ...
    # Machine-style title casing inside sentences seen in the realm bonus section.
    for a,b in [(' Sind ',' sind '),(' Ist ',' ist '),(' Im Nächsten Kampf',' im nächsten Kampf'),(' Zufälligen ',' zufälligen '),(' Zufälliger ',' zufälliger '),(' Mit Einem ',' mit einem '),(' Belegen ',' belegen '),(' Gewähre ',' gewähren '),(' Auf',' auf')]:
        out=out.replace(a,b)
    # Keep chosen difficulty label consistent in its own explanatory text.
    if '"Ruthless"' in en: out=out.replace('"Gnadenlos"','"Unbarmherzig"')
    # Natural wording for Skip Story Mode descriptions.
