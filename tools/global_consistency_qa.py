...
def main():
 ...
 for fn,line,en,de in rows:
  ...
  acts=sorted(set(ACTION.findall(en)+ACTION.findall(de)),key=str.lower)
  for act in acts:
   ec=en.count(act);dc=de.count(act);action_counts[act]+=max(ec,dc);action_rows.append((act,fn,line,ec,dc,en,de))
